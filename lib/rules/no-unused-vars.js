 ...
 * @typedef {'array-destructure'|'catch-clause'|'parameter'|'variable'|'type'} VariableType
 */
...
module.exports = {
	...
	create(context) {
		...
		function defToVariableType(def) {
			...
			// TypeScript interface, type alias, or enum
			if (
				def.name.parent.type === "TSInterfaceDeclaration" ||
				def.name.parent.type === "TSTypeAliasDeclaration" ||
				def.name.parent.type === "TSEnumDeclaration"
			) {
				return "type";
			}
		...
		/**
		 * Finds a reference in the given scope.
		 * @param {Scope} scope The scope to find.
		 * @param {ASTNode} node The node to find.
		 * @returns {Reference|null} The reference object.
		 * @private
		 */
		...
		/**
		 * Checks whether a reference is a read operation for itself or part of it
		 * (e.g. ++a, a += 1, a = a + 1, a = a[0] + 1).
		 * @param {Reference} ref The reference to check.
		 * @param {ASTNode|null} rhsNode The RHS node of the previous reference.
		 * @returns {boolean} True if the reference is a read operation for itself.
		 * @private
		 */
		function isReadForItself(ref, rhsNode) {
			...
			/*
			 * When the reference is in the right of an assignment and it's
			 * in a node of a previous reference's right side node,
			 * this reference is for itself.
			 *
			 * e.g.:
			 *   a = a + 1;
			 *   a = (a = 1, a);
			...
			if (parent.type === "AssignmentExpression") {
				if (id === parent.left) {
					/*
					 * This reference is in the left of an assignment.
					 * If the right is a reference to this variable, this is a self-reference.
					 * e.g.: a = a;
					 */
					if (
						parent.right.type === "Identifier" &&
						parent.right.name === id.name
					) {
						return true;
					}

					// If the right contains this variable, this is a self-reference.
					// e.g.: a = a + 1; a = [a]; a = foo(a);
					// Except if it's in object property.
					// e.g.: a = { foo: a };
					const rightRef = findReference(ref.from, parent.right);

					if (
						rightRef &&
						!isMemberAccessOnObject(rightRef.identifier)
					) {
						return true;
					}
				}

				if (id === parent.right && grandparent.type === "ExpressionStatement") {
					// e.g.: a = a;
					if (
						parent.left.type === "Identifier" &&
						parent.left.name === id.name
					) {
						return true;
					}
				}
			...
			// These cases are simple self-references but they are read operations:
			// e.g.: a += 1; a++; a = a + 1;
			if (
				(parent.type === "AssignmentExpression" &&
					parent.operator !== "=" &&
					id === parent.left) ||
				(parent.type === "UpdateExpression" && id === parent.argument)
			) {
				return false;
			}
		...
		/**
		 * Determines if the variable is defined in a TypeScript declaration file
		 * @param {Variable} variable eslint-scope variable object
		 * @returns {boolean} True if the variable is in a declaration file, false if not
		 * @private
		 */
		function isInTypeScriptDeclarationFile(variable) {
			if (!context.filename) {
				return false;
			}

			// Check if file ends with .d.ts
			return context.filename.endsWith(".d.ts");
		}

		/**
		 * Determines if the variable is in an ambient declaration
		 * @param {Variable} variable eslint-scope variable object
		 * @returns {boolean} True if the variable is in an ambient declaration, false if not
		 * @private
		 */
		function isInAmbientDeclaration(variable) {
		...
		/**
		 * Check if a reference is in a TypeScript type annotation
		 * @param {Reference} ref The reference to check
		 * @returns {boolean} Whether the reference is in a TypeScript type annotation
		 * @private
		 */
			...
			while (parent) {
				switch (parent.type) {
					...
					case "FunctionDeclaration":
						// These nodes have type annotations as child nodes, 
						// but the identifier itself might not be in the type annotation
						return false;
					default:
						parent = parent.parent;
				}
		...
		/**
		 * Checks whether the given variable is only used as a type
		 * @param {Variable} variable The variable to check
		 * @returns {boolean} True if the variable is only used as a type
		 * @private
		 */
		function isOnlyUsedAsType(variable) {
			// If there are no references, it's not used as a type
			if (variable.references.length === 0) {
				return false;
			}

			// Check if all references are in type annotations
			return variable.references.every(ref => isTypeReference(ref));
		}
			...
			if (scope.type !== "global" || config.vars === "all") {
				for (i = 0, l = variables.length; i < l; ++i) {
					...
					if (
						...
						// Handle variables that are only used in TypeScript type annotations
						if (isOnlyUsedAsType(variable)) {
							context.report({
